...
class PplDocLevelPromptsDataset(PromptsDataset):
    ...
    def get_vals(self, i, query=""):
        ...
        if n_from_ds1 > 0:
            ...
            with torch.no_grad():
                test_sent = self.ds2.df.iloc[i]['source']
                inp0 = torch.LongTensor(self.tokenizer.encode(test_sent)).cuda()
                lab0 = inp0
                loss0 = self.model(inp0, labels=lab0).loss

            j=1
            selected_ids = []
            while len(selected_ids) < self.nprompts+1 and j<len(ds2_s):
                j1 = ds2_s.iloc[-j]['source']
                inpj = torch.LongTensor(self.tokenizer.encode(j1)).cuda()
                labj = torch.LongTensor([-100] * len(inpj)).cuda()

                inp = torch.concat([inpj, inp0])
                lab = torch.concat([labj, lab0])
                with torch.no_grad():
                    lossj = self.model(inp, labels=lab).loss

                if torch.exp(loss0) - torch.exp(lossj) > 1:
                    selected_ids.append(ds2_s.iloc[-j]['id'])
                    #print("included:", ds2_s.iloc[-j]['source'])
                #else:
                    #print("rejected:", ds2_s.iloc[-j]['source'])

                j += 1
            
            #vals = ds2_s.iloc[-n_from_ds2:][[self.skey, tkey2]]
            vals = ds2_s[ds2_s['id'].isin(selected_ids[::-1])][[self.skey, tkey2]]

            if len(vals) < self.nprompts:
                n_from_ds1 = self.nprompts - len(vals)
                vals1 = self.ds1.df_sample.sample(n=n_from_ds1, random_state=self.seed)
                vals = pd.concat([vals1[[self.skey, tkey2]], vals])
