...
# take in two datasets, the first will be used to prompt, the second will be used to test
        ...
        # self.prefix_compress = self.get_prefix(self.ds1, self.seed, compress=True)
        # test with only 200 examples for efficiency
        self.ntest = ntest
    ...
    def get_prefix(self, ds, seed=None, compress=False, source_only=False, target_only=False, i=0):
        ...
        #if type(self.a) != str:
        #    prefix = f"{self.sep}".join([f"{self.q}{v[0]}{self.a[i]}{v[1]}" for i, v in enumerate(vals)])
        #else:
    ...
    def _shufflevariants(self, vals):
        ...
        if "order" in self.shuffle_mode:
            # shuffle the order
            # but we join source and target with ":::" first.
        ...
        return vals
    # create a shuffle such that the correct source is always one after the target. 
    # create a shuffle such that the correct target is always one after the source. 
    ...
    def __getitem__(self, i):
        ...
        if type(self.a) == str:
            ...
            query = f"{self.sep}{self.q}{source}{self.a[0]}" 

        # inference time input
        total_input = f"{self.prefix}{query}"
        ...
        item = {"id": self.ds2[i]['id'],
                ...
                "input2": total_input2,
                "prompt": self.prefix,
                #"prefix_compress": self.prefix_compress,
                "query": query,
                "query_raw": source,
                "target": self.ds2[i]['target'] + self.eos}
        #item = {"input": prompt, "target": target, "id": self.ds2[i]['id'],
        #       "prefix": self.prefix}
        #       "query": f"\nQ: {self.ds2[i]['source']} A: "}
